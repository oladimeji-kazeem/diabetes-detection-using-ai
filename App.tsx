...
import { HealthData, AssessmentRecord, RiskAssessment, UserRole, UserProfile } from './types';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'assess' | 'history' | 'assistant' | 'learn' | 'manage' | 'profile' | 'workflow'>('assess');
  const [loading, setLoading] = useState(false);
  const [currentResult, setCurrentResult] = useState<RiskAssessment | null>(null);
  const [records, setRecords] = useState<AssessmentRecord[]>([]);
  const [profile, setProfile] = useState<UserProfile>(() => {
    const saved = localStorage.getItem('glucoscan_profile');
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4 transition-colors duration-300">
        <div className="max-w-7xl w-full">
          <div className="text-center mb-12 animate-in fade-in zoom-in duration-700">
            ...
            <h1 className="text-5xl font-black text-[#1E3A8A] dark:text-blue-400 mb-4 tracking-tight">GlucoScan Network</h1>
    ...
    <div className={`min-h-screen flex flex-col md:flex-row ${profile.preferences.darkMode ? 'dark' : ''}`}>
      <nav className="w-full md:w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 p-6 flex flex-col gap-2 no-print shadow-sm z-20 transition-colors duration-300">
        <div className="flex items-center gap-3 px-2 py-4 mb-8">
          ...
          <h1 className="text-2xl font-black text-[#1E3A8A] dark:text-white tracking-tight">GlucoScan</h1>
        </div>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 md:px-12 md:py-12 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-8">
            {activeTab === 'workflow' && <WorkflowOrchestrator role={role} records={records} setRecords={(recs) => { setRecords(recs); localStorage.setItem('glucoscan_records', JSON.stringify(recs)); }} />}
            
            {activeTab === 'profile' && <UserProfileSection profile={profile} onSave={(p) => { setProfile(p); localStorage.setItem('glucoscan_profile', JSON.stringify(p)); }} />}
